...
export function add_event_listeners(source: API.APIEventTarget) {
    ...
    source.addEventListener('phase_switcher/config', () => API.default_updater('phase_switcher/config'));
    source.addEventListener('meter/values', () => update_meter_values());
}

export function init() {
    console.log("Phase switcher init")
...
function phase_switcher_chart_change_time() {
    if (graph_update_interval != null) {
        clearInterval(graph_update_interval);
        graph_update_interval = null;
    }
...
function init_chart() {
    ...
    phase_switcher_chart_change_time();
}